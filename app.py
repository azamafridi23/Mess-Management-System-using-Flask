    ...
    if user_type not in ["Student",'Supervisor']:
        ...
        try:
            ...
            if conn_ld is not None and conn_bf is not None:
                ...
                # Execute the query with user_id=1
                cursor_bf.execute(sql_query, (user_id,f"{2024}-{month:02d}"))
                ...
                cursor_ld = conn_ld.cursor()
                sql_query2 = "SELECT * FROM LD WHERE User_id = ? AND strftime('%Y-%m', Date) = ?"
                # Execute the query with user_id=1
                cursor_ld.execute(sql_query2, (user_id,f"{2024}-{month:02d}"))
                result_ld = cursor_ld.fetchall()
